...
============================================================*/
	...
	gulp.task('copy', ['copy:jade', 'copy:images', 'copy:fonts', 'copy:jade:root']);


	gulp.task('copy:jade', function () {

		console.log('-------------------------------------------------- COPY :jade');
		gulp.src([settings.src.templates + '*.jade', settings.src.templates + '**/*.jade'])
            .pipe(jade())
	...
	gulp.task('copy:jade:root', function() {

		console.log('-------------------------------------------------- COPY :jade:root');
		gulp.src(settings.src.app + '*.jade')
            .pipe(jade())
			.pipe(gulpif(isProduction, minifyHTML({comments: false, quotes: true, spare:true, empty: true, cdata:true})))
...
=========================================================================================================*/
	...
	gulp.task('watch', function () {
		...
		var templateFiles = gulp.watch([settings.src.templates + '*.jade', settings.src.templates + '**/*.jade'], ['copy:jade']);
...
============================================================*/
	...
	gulp.task('clean', function () {
		...
		// gulp.run('clean:css', 'clean:js', 'clean:jade', 'clean:images', 'clean:fonts');
...
============================================================*/
	...
	gulp.task('sync', function() {
	    browserSync.init([settings.build.app + 'index.jade', settings.build + 'templates/*.jade', settings.build.css + '*.css', settings.build.js + '*.js'], {
	         proxy: {
	            host: '127.0.0.1',
